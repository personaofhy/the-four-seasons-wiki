...
## 🎨 2段パイプライン — イラスト調は「仕様」、リアル調は「成果物」

**最終的な画風はリアル調。**ただしイラスト調は捨てない。**特徴を人間とAIで明示的に整理するための中間段階**として使う。

| | 段階1：イラスト調 | 段階2：リアル調 |
| :--- | :--- | :--- |
| **役割** | **仕様の合意** | **最終出力** |
| **狙い** | 冷や汗の粒・円形の赤面・光の消えた瞳などの**記号を誇張して描き**、何が決まったかを双方が確認できる状態にする | 合意済みの特徴を写実に落とす |
| **Checkpoint** | `Counterfeit-V3.0_fix_fp16` | `dreamshaper_8` |
| **ワークフロー** | `char_ipadapter` | `char_ipadapter_controlnet` |
| **同一性参照** | イラスト調の顔 | **リアル調の顔** |
| **構図・表情の受け渡し** | — | **段階1の出力から `canny_map` / `depth_map` を取り ControlNet へ** |

> [!warning] 段階2の参照画像にイラストを使ってはならない
> IP-Adapterは**参照画像の画風を引き継ぐ**。段階1のイラストを段階2の参照に使うと、出力がイラストに引き戻される。
> **画風は参照画像で決まり、プロンプトでは変えられない。**
>
> 段階1から段階2へ渡すのは**画風ではなく構造**（canny／depth）。同一性はリアル調の参照から取る。

> [!danger] 橘柊の段階2には制約がある
> 柊は19歳だが、**幼い顔立ち・幼児体型寄り・子供への退行**という設計を意図的に持っている。
> 写実の画風でこの設計を回すと、**出力が未成年に見える**ことが起こりうる。
>
> **段階2では、顔の骨格と身体のプロポーションを明確に成人のものとして保つこと。**
> 満たさない出力は調整せず破棄する。柊の幼さは**服装と振る舞いで出すのであって、顔や身体を子供に寄せて出すのではない。**
