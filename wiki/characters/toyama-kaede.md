...
birthday: 10-10
blood_type: AB
occupation: 会社員（大手企業 総合職）
hair: 温かみのある深いブラウン寄りの黒のロングストレート（勤務時は低いシニヨン／夜会巻き）
eyes: クリンとした黒目がちの離れ目、やや垂れ目。明るめブラウン
body_type: 細身で繊細。線が細く脚が長いモデル体型
outfit_main: 紺碧のVネックラップワンピース（マットウール・ミディ丈フレア）／黒レザーベルト
underwear: ラベンダーのレース上下セット
legwear: 20〜30デニールのパンティストッキング（ダークネイビー〜ブラック）
shoes: ダークブラウン本革のポインテッドトゥ・パンプス
social_persona: 知的で隙のない完璧な憧れの先輩・女神
inner_flaw: 権威とプライドによる全能感、他者への見下し
breakdown_trigger: キャンプ場の泥の中での漏濁と屈服
adaptation_type: 立場逆転と服従（後輩への屈服と支配の受け入れ）
aesthetic: 美貌と権威の失墜
agency: 強制
symbol_item: 泥と濡れたストッキング／落ち葉の中のダークブラウンのパンプス／洗面台で洗われる紫ショーツ
likes: [紅茶, 読書, クラシック音楽]
dislikes: [騒がしい場所, 時間に追われること]
skills: [フランス語, ピアノ, ワインを嗜むこと]
quirk: ストッキングは常に予備を携帯
animal_motif: 子鹿（秋の深山）
face_model: 小西真奈美風の柔らかさ
color_main: ["#1E283A", "#622426", "#523D2C", "#9F9085"]
...
第3章「秋」の主人公。26歳、大手企業の総合職。憧れの対象から、後輩に秘密を握られる存在へ墜落する。

**「侵しがたい女神」を成立させるための、隙のない外貌**を持つ。顔は和風の知性美、肉体は8頭身のスレンダー、服装は紺碧のワンピースで統一された「完成された服」。下着に至るまでラベンダーで揃っているのは、**誰にも見せない場所まで完璧である**という自負の表現であり、第3章の失墜で最も強く裏返る部分でもある。
...
| 項目 | 設定 |
| :--- | :--- |
| **動物モチーフ** | 🦌 **子鹿 — 秋の深山**。つぶらで**少し離れ目**、繊細な愛嬌がある。ただし*見た目の可憐さと裏腹に性格は少し怖い*という落差そのものが、この人物の設計思想 |
| **輪郭** | **面長。4人中もっとも小顔**。細い顎、シャープすぎない柔らかい線 |
| **髪色・質感** | 濡れたような艶のある**温かみのある深いブラウン寄りの黒**（イエベ秋／オータムライトで濃いブラウンに転ぶ） |
| **髪型（素）** | ロングストレート。自然な分け目、鎖骨より下まで |
| **髪型（勤務時）** | **低い位置でまとめた夜会巻き／シニヨン**。後れ毛を許さない端正さ |
| **眉** | オータムカラーの濃いめブラウン、**垂れ眉**。柔和さの源 |
| **目** | **クリンとした黒目がちの離れ目、やや垂れ目**。子鹿風の澄んだ瞳、明るめブラウン。睫毛は長く自然。**離れ目は4人中唯一で、最大の識別点** |
| **鼻・口** | 通った細い鼻筋。薄めの上唇とふっくらした下唇、コーラルレッドのリップ |
| **肌** | 色白、きめ細かくマット |
| **雰囲気** | 柔らかさと、高学歴先輩の**神経質さが同居** |
| **コントラスト** | **中程度。**深いブラウンの髪と明るめブラウンの瞳、色白の肌。4段階では上から2番目 |
| **赤面・生理表現** | 鼻梁と耳たぶが赤くなり、プライドで必死に抑圧する |

> [!tip] パーソナルカラーは「秋」— 章の季節と一致
> 温かみのある深いブラウン、マットな肌、落ち着いた重厚さ。既存設定の「オータムライトで濃いブラウンに転ぶ黒髪」「陶器のような白さ、マットな質感」がそのままイエベ秋の記述になっている。
> **こっくりとした重厚さが「侵しがたい女神」の格を支える**一方、第3章で泥をかぶったとき、同系色の泥に飲み込まれて**輪郭を失う**。秋のパレット（`#523D2C` `#997254` `#AE9A7F`）と泥の色が近いことが、失墜の絵をつくる。

> [!warning] 髪型は「ロング」が正
> 着用イメージボード `kaede_fullbody_base.jpg` と設定シート三面図は**ボブ**で描かれているが、これは誤り。顔の確定素材15枚がすべてロングであるため、**画像資料の側を差し替える**。

> [!warning] 眉は「垂れ眉・柔和」が正 — 鉛筆スケッチは差し替え対象
> `references/kaede_pencil_sketch.png` は**太い直線的な眉・無表情でクール**に描かれており、設定書の「垂れ眉」「柔和さの源」「小西真奈美風の柔らかさ」と**ほぼ真逆**になっている。
> **設定書を正とし、画像資料の側を差し替える。** スケッチで唯一設定と合致していた**やや離れ目**のみ維持する。
>
...
| 項目 | 設定 |
| :--- | :--- |
| **身長・等身** | 165cm / **8頭身**。4人中最も高い等身比 |
| **体型** | 健康的スレンダー。脂肪も筋肉も少なく、**線の細さが威圧感に転じる**タイプ |
| **バスト** | 小さめ〜控えめ。豊満さではなく薄さで構成 |
| **ウエスト・ヒップ** | 明確なくびれ。臀部は**膨らみすぎない細身の丸み** |
| **四肢** | 長い手足、細い手首・足首。指も長くしなやか |
| **肌** | 陶器のような白さ、マットな質感。日焼けの痕跡なし |
| **背面** | **肩甲骨と背骨のラインがくっきり出る**。うなじが美しく、まとめ髪と対になる |
| **足** | 甲の薄い細身の足。ハイヒールが映える形状 |

## 👗 服装

👉 **[[toyama-kaede-outfit|👗 遠山 楓 衣装詳細・下着破綻変遷シートを見る]]**
...
| **ベルト** | 黒レザー、**アンティークシルバーのバックル**。余った帯が垂れる |
| **鞄** | 小さめの**ダークブラウン本革**レザーハンドバッグ（フラップ＋ゴールド金具） |
| **靴** | **ダークブラウン本革**のポインテッドトゥ・パンプス。ヒール7〜8cm、装飾なし |
| **アクセサリー** | 小ぶりのパールピアスのみ。指輪・ネックレスなし |
| **代替衣装** | 上質なオフィスカジュアル／エレガントなパンツスーツ |

### 下着・レッグウェア

| 部位 | 設定 |
| :--- | :--- |
| **ブラジャー** | 薄紫（ラベンダー）のレース、ワイヤー入り。カップ全面に**枝葉状の花柄レース**、裾は**スカラップレース**、細いサテンストラップ。中央に小さなリボンと**ゴールドの雫型チャーム** |
| **ショーツ** | ブラと完全にお揃い。**サイドは細い三本ストラップ**、スカラップの縁取り、前中央に同じリボン＋チャーム。ローライズ、シースルーのレースパネル |
| **レッグウェア** | **20〜30デニールの高級パンティストッキング**（一体型）。ダークネイビー〜ブラック。薄く均一な透け感。**常に予備を鞄に携帯している** |

> [!tip] 「上下セット」であることの意味
> 誰にも見せない下着まで色とレースを揃えているという事実が、**自負が仮面ではなく生活習慣そのもの**であることを示す。だからこそ、それが泥に沈み後輩の手で洗面台で洗われる展開が、単なる羞恥ではなく**アイデンティティの解体**として機能する。
>
> 破れることを想定して予備まで携帯しているのに、第3章では**替える猶予すら与えられずに膝を折る**。用意周到さが無意味になる瞬間が、この人物の失墜の核心にある。
...
| :--- | :--- |
| **通常** | 完璧な高学歴の女神の冷笑 |
| **抑圧・冷や汗** | 鼻梁を赤くし、眉を寄せてプライドを抑圧 |
| **狼狽・大赤面** | 女神の仮面が完全に砕けた激しい混乱 |
| **虚脱・完全適応** | 後輩男子を見上げる屈服と従順な瞳 |
...
### 1. 【高慢】絶対的優位の仮面
洗練されたワンピースとストッキングに身を包み、後輩に対して圧倒的な先導者として君臨する。

### 2. 【墜落】紅葉の泥の中
便所の目前で耐え切れず絶望の失禁。高級なストッキングが泥と液体で重く変色し、落ち葉の上で膝を折る。

### 3. 【服従】支配権の譲渡
完璧な先輩としての誇りが完全に折れ、後輩に弱みを握られ「尻拭い」をされる中で、支配される側に回る安堵を得る。
...
> [!example]- 肉体（Body / Anatomy Reference）
> ```text
...
> [!example]- 服装（Outfit Reference / ゴーストマネキン）
> ```text
...
生成手順とパラメータは [[rules/comfyui-mcp-workflows|ComfyUI MCP用 ワークフロー規定]] を参照。**チェックポイントは `dreamshaper_8`**（アニメモデルでは確定素材の画風と合わない）。
...
### 画像資料の修正待ち
- [ ] 着用イメージボードと設定シート三面図の髪型を**ボブ→ロング**へ

### 統合時チェックリスト
- [ ] 素体の等身と衣装の着丈比率が合致しているか
- [ ] 光源がフラットなスタジオ光で一貫しているか
- [ ] 首・手首・足首の接合部で肌色やテクスチャの乖離がないか
- [ ] シーンが勤務中ならまとめ髪、私的な場面ならロングになっているか
