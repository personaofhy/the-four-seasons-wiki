---
title: "遠山 楓"
type: character
character_id: toyama-kaede
name_ja: 遠山 楓
name_kana: とおやま かえで
season: 秋
role: 章主人公
featured_chapter: 03-autumn
age: 26
height_cm: 165
head_ratio: 8.0
birthday: 10-10
blood_type: AB
occupation: 会社員（大手企業 総合職）
hair: 黒髪のエレガントなロングストレート（勤務時は低いシニヨン／夜会巻き）
eyes: 明るいブラウンの子鹿風の澄んだ瞳
body_type: スレンダーで脚が長く凛としたモデル体型
outfit_main: 紺碧のVネックラップワンピース（マットウール・ミディ丈フレア）／黒レザーベルト
underwear: ラベンダーのレース上下セット
legwear: 20〜30デニールのパンティストッキング（ダークネイビー〜ブラック）
shoes: ダークブラウン本革のポインテッドトゥ・パンプス
social_persona: 知的で隙のない完璧な憧れの先輩・女神
inner_flaw: 権威とプライドによる全能感、他者への見下し
breakdown_trigger: キャンプ場の泥の中での漏濁と屈服
adaptation_type: 立場逆転と服従（後輩への屈服と支配の受け入れ）
aesthetic: 美貌と権威の失墜
agency: 強制
symbol_item: 泥と濡れたストッキング／落ち葉の中のダークブラウンのパンプス／洗面台で洗われる紫ショーツ
likes: [紅茶, 読書, クラシック音楽]
dislikes: [騒がしい場所, 時間に追われること]
skills: [フランス語, ピアノ, ワインを嗜むこと]
quirk: ストッキングは常に予備を携帯
art_style: Photorealistic / Semi-real 3D render
status: designed
tags: [character, season/autumn]
---

# 🍁 遠山 楓（とおやま かえで）

第3章「秋」の主人公。26歳、大手企業の総合職。憧れの対象から、後輩に秘密を握られる存在へ墜落する。

**「侵しがたい女神」を成立させるための、隙のない外貌**を持つ。顔は和風の知性美、肉体は8頭身のスレンダー、服装は紺碧のワンピースで統一された「完成された服」。下着に至るまでラベンダーで揃っているのは、**誰にも見せない場所まで完璧である**という自負の表現であり、第3章の失墜で最も強く裏返る部分でもある。

---

## 🎭 顔

| 項目 | 設定 |
| :--- | :--- |
| **輪郭** | 卵型〜やや面長。細い顎、シャープすぎない柔らかい線 |
| **髪色・質感** | 濡れたような艶のある黒髪（オータムライトで濃いブラウンに転ぶ） |
| **髪型（素）** | ロングストレート。自然な分け目、鎖骨より下まで |
| **髪型（勤務時）** | **低い位置でまとめた夜会巻き／シニヨン**。後れ毛を許さない端正さ |
| **眉** | オータムカラーの濃いめブラウン、**垂れ眉**。柔和さの源 |
| **目** | **子鹿風の澄んだ瞳**。明るめブラウン、やや大きめの二重。睫毛は長く自然 |
| **鼻・口** | 通った細い鼻筋。薄めの上唇とふっくらした下唇、コーラルレッドのリップ |
| **肌** | 色白、きめ細かくマット |
| **雰囲気** | 柔らかさと、高学歴先輩の**神経質さが同居** |
| **赤面・生理表現** | 鼻梁と耳たぶが赤くなり、プライドで必死に抑圧する |

> [!warning] 髪型は「ロング」が正
> 着用イメージボード `kaede_fullbody_base.jpg` と設定シート三面図は**ボブ**で描かれているが、これは誤り。顔の確定素材15枚がすべてロングであるため、**画像資料の側を差し替える**。

## 🦴 肉体

| 項目 | 設定 |
| :--- | :--- |
| **身長・等身** | 165cm / **8頭身**。4人中最も高い等身比 |
| **体型** | 健康的スレンダー。脂肪も筋肉も少なく、**線の細さが威圧感に転じる**タイプ |
| **バスト** | 小さめ〜控えめ。豊満さではなく薄さで構成 |
| **ウエスト・ヒップ** | 明確なくびれ。臀部は**膨らみすぎない細身の丸み** |
| **四肢** | 長い手足、細い手首・足首。指も長くしなやか |
| **肌** | 陶器のような白さ、マットな質感。日焼けの痕跡なし |
| **背面** | **肩甲骨と背骨のラインがくっきり出る**。うなじが美しく、まとめ髪と対になる |
| **足** | 甲の薄い細身の足。ハイヒールが映える形状 |

## 👗 服装

### 通常時 — 完成された服（知性・支配）

| 部位 | 設定 |
| :--- | :--- |
| **ワンピース** | 紺碧（ディープネイビー）の**Vネック・ラップワンピース**。長袖、ミディ丈のフレアスカート |
| **素材** | マットなウール。光沢なし、柔らかいネップのある質感 |
| **ベルト** | 黒レザー、**アンティークシルバーのバックル**。余った帯が垂れる |
| **鞄** | 小さめの**ダークブラウン本革**レザーハンドバッグ（フラップ＋ゴールド金具） |
| **靴** | **ダークブラウン本革**のポインテッドトゥ・パンプス。ヒール7〜8cm、装飾なし |
| **アクセサリー** | 小ぶりのパールピアスのみ。指輪・ネックレスなし |
| **代替衣装** | 上質なオフィスカジュアル／エレガントなパンツスーツ |

### 下着・レッグウェア

| 部位 | 設定 |
| :--- | :--- |
| **ブラジャー** | 薄紫（ラベンダー）のレース、ワイヤー入り。カップ全面に**枝葉状の花柄レース**、裾は**スカラップレース**、細いサテンストラップ。中央に小さなリボンと**ゴールドの雫型チャーム** |
| **ショーツ** | ブラと完全にお揃い。**サイドは細い三本ストラップ**、スカラップの縁取り、前中央に同じリボン＋チャーム。ローライズ、シースルーのレースパネル |
| **レッグウェア** | **20〜30デニールの高級パンティストッキング**（一体型）。ダークネイビー〜ブラック。薄く均一な透け感。**常に予備を鞄に携帯している** |

> [!tip] 「上下セット」であることの意味
> 誰にも見せない下着まで色とレースを揃えているという事実が、**自負が仮面ではなく生活習慣そのもの**であることを示す。だからこそ、それが泥に沈み後輩の手で洗面台で洗われる展開が、単なる羞恥ではなく**アイデンティティの解体**として機能する。
>
> 破れることを想定して予備まで携帯しているのに、第3章では**替える猶予すら与えられずに膝を折る**。用意周到さが無意味になる瞬間が、この人物の失墜の核心にある。

### 破綻時（第3章・キャンプ場）

| 部位 | 状態 |
| :--- | :--- |
| **ストッキング** | **伝線**し、泥と尿でぐっしょりと重く濡れそぼる。膝から下が茶色く変色 |
| **靴** | ダークブラウンのパンプス。**片方が泥に埋もれ**、落ち葉の上に転がる |
| **ショーツ** | ラベンダーのレースショーツ。**洗面台で（後輩の手により）洗われる** |
| **ワンピース** | 裾に泥が跳ね、膝をついた位置に濡れ染みが残る |

## 😶 表情パターン

共通定義は [[characters/index|キャラクター一覧]] を参照。

| パターン | 楓の描き分け |
| :--- | :--- |
| **通常** | 完璧な高学歴の女神の冷笑 |
| **抑圧・冷や汗** | 鼻梁を赤くし、眉を寄せてプライドを抑圧 |
| **狼狽・大赤面** | 女神の仮面が完全に砕けた激しい混乱 |
| **虚脱・完全適応** | 後輩男子を見上げる屈服と従順な瞳 |

---

## 🗣️ 声・話し方

（未定義）

## 🤝 人物相関

（未定義）

## 🎞️ 章別の登場

（未定義）

## 🏠 生活背景

（未定義）

---

## 📉 心理的軌道と崩壊プロセス

### 1. 【高慢】絶対的優位の仮面
洗練されたワンピースとストッキングに身を包み、後輩に対して圧倒的な先導者として君臨する。

### 2. 【墜落】紅葉の泥の中
便所の目前で耐え切れず絶望の失禁。高級なストッキングが泥と液体で重く変色し、落ち葉の上で膝を折る。

### 3. 【服従】支配権の譲渡
完璧な先輩としての誇りが完全に折れ、後輩に弱みを握られ「尻拭い」をされる中で、支配される側に回る安堵を得る。

---

## 🖼️ 確定素材

### 顔

| 決定版 正面顔 | 統合デザインシート（3-in-1） |
| :---: | :---: |
| ![顔・決定版](../assets/base-ai/kaede_face_front_base.jpg) | ![統合シート](../assets/base-ai/kaede_unified_design_sheet.jpg) |

検討バリエーション（アーカイブ）：`kaede_face_cropped_base` / `kaede_face_autumn_b・c` / `kaede_face_c1〜c3` / `kaede_face_konishi_a〜c` / `kaede_face_var_a〜c`

### 肉体

| Tポーズ 正面 | Tポーズ 背面 | 深度マップ |
| :---: | :---: | :---: |
| ![素体・正面](../assets/base-ai/kaede_body_tpose_base.jpg) | ![素体・背面](../assets/base-ai/kaede_body_tpose_back_base.jpg) | ![深度マップ](../assets/depth-maps/kaede_body_tpose_depth.png) |

その他：`kaede_fullbody_nude_perfect.jpg`（決定版）／`kaede_fullbody_nude_base.jpg`／`kaede_fullbody_nude_comfyui.png`

### 服装

| ワンピース単体（**推奨参照**） | 着用状態（原典クロップ） | 下着セット |
| :---: | :---: | :---: |
| ![衣装単体](../assets/references/kaede_dress_navy_ref.png) | ![着用状態](../assets/references/kaede_dress_navy_worn_ref.png) | ![下着](../assets/references/kaede_lingerie_lavender_ref.png) |

いずれも**顔を含まない**ため、キャラ生成時に別人の顔が混入しない。全て ComfyUI の input にアップロード済み。

### ドット絵

![立ち姿](../assets/strict-16colors/kaede_fullbody_strict_16colors.png)

---

## 🤖 生成用プロンプト

> [!example]- 顔（Face Reference）
> ```text
> photorealistic, semi-realistic 3d character render, extreme close-up portrait of a japanese woman, Kaede Toyama, long straight glossy black hair, centre parted, soft drooping eyebrows in dark autumn brown, clear deer-like light brown eyes, double eyelids, long natural lashes, slender straight nose, thin upper lip and full lower lip, coral red lipstick, fair matte porcelain skin, refined intellectual and slightly nervous expression, plain white background, soft even studio lighting, high detail
> ```
> **Negative**: `anime, cel shading, cartoon, big anime eyes, short hair, bob cut, brown hair, blonde, heavy makeup, smiling, freckles, blemishes, text, watermark`

> [!example]- 肉体（Body / Anatomy Reference）
> ```text
> photorealistic, semi-realistic 3d character render, soft matte skin, full body shot of an unclothed slender japanese woman, T-pose, standing straight, anatomically accurate proportions, 8 heads tall, tall and slim, small bust, defined waist, long limbs, thin wrists and ankles, fair porcelain skin, visible shoulder blades and spine line, no tan lines, plain white background, flat even studio lighting, anatomy reference sheet
> ```
> **Negative**: `clothes, underwear, large breasts, muscular, overweight, anime, cel shading, cartoon, dramatic lighting, shadows, text, watermark`

> [!example]- 服装（Outfit Reference / ゴーストマネキン）
> ```text
> photorealistic, professional e-commerce product photograph, a deep navy blue V-neck long sleeve wrap dress, black leather belt with antique silver buckle at the waist, plain midi flared A-line skirt, matte navy wool fabric, soft napped texture, no sheen, ghost mannequin, invisible person, entire dress from shoulders to hem fully visible, detailed fabric texture, visible seams, isolated on pure white seamless background, high key even studio lighting, catalog photo
> ```
> **Negative**: `person, face, head, hair, skin, shiny, glossy, latex, satin, leather dress, brown belt, buttons, dark background, room, furniture, anime, cartoon, text, watermark`

生成手順とパラメータは [[rules/comfyui-mcp-workflows|ComfyUI MCP用 ワークフロー規定]] を参照。**チェックポイントは `dreamshaper_8`**（アニメモデルでは確定素材の画風と合わない）。

---

## 📋 制作メモ

### 不足素材
- [ ] 顔・**側面プロファイル**（マルチアングル化）
- [ ] 顔・**まとめ髪バージョン**（勤務時の正面／うなじ）
- [ ] 肉体・**サイドビュー**（現状は正面・背面のみ）
- [ ] 服装・**背面**（ゴーストマネキン）
- [ ] **レッグウェア＋靴 単体**（パンスト・ダークブラウンのパンプス）
- [ ] **破綻状態の衣装参照**（伝線・泥・濡れ）

### 画像資料の修正待ち
- [ ] 着用イメージボードと設定シート三面図の髪型を**ボブ→ロング**へ

### 統合時チェックリスト
- [ ] 素体の等身と衣装の着丈比率が合致しているか
- [ ] 光源がフラットなスタジオ光で一貫しているか
- [ ] 首・手首・足首の接合部で肌色やテクスチャの乖離がないか
- [ ] シーンが勤務中ならまとめ髪、私的な場面ならロングになっているか
